...
ul.relative.text-center
  li.relative.overflow-hidden(v-for="(social,index) in sections.contact.socials" :key="social" style="height:55px")
    a.p-4.rounded-2xl.inline-block.transition-margin.duration-500(:style="{ marginTop: mounted ? '0px' : '56px', transitionDelay: `${index * 100}ms` }" :href="social" target="_blank" style="height:56px" :title="social" class="hover:text-indigo-600" v-if="socialInfo(social)")
